...
[![GitHub stars](https://img.shields.io/github/stars/sjkncs/daily_stock_analysis?style=social)](https://github.com/sjkncs/daily_stock_analysis/stargazers)
...
[**功能特性**](#-功能特性) · [**Web 界面**](#️-web-界面) · [**快速开始**](#-快速开始) · [**推送效果**](#-推送效果) · [**完整指南**](docs/full-guide.md) · [**常见问题**](docs/FAQ.md)
...
```bash
# 克隆项目
git clone https://github.com/sjkncs/daily_stock_analysis.git && cd daily_stock_analysis
...
现代化 React + FastAPI 全栈 Web UI，采用 **千问/豆包** 风格设计，支持亮/暗双主题。

### Web UI 功能模块

| 页面 | 路由 | 功能说明 |
|------|------|----------|
| 首页 | `/` | AI 投研仪表盘，工具入口，股票代码快捷分析 |
| 智能分析 | `/analysis` | 触发 AI 分析任务，SSE 实时推送，历史报告查看 |
| 技术面分析 | `/tools/technicals` | K线数据、SMA/RSI 指标计算、区间统计、波动率 |
| 舆情分析 | `/tools/sentiment` | AI 新闻情绪评分、操作建议、趋势预测 |
| 条件选股 | `/tools/screener` | 批量行情获取、多维排序、一键跳转分析 |
| 个股对比 | `/tools/compare` | 双股票实时行情横向对比 |
| 智能预警 | `/tools/alert` | 价格/涨跌幅预警规则管理 |
| 大盘复盘 | `/market` | 指数走势、板块排行、北向资金 |
| AI 回测 | `/backtest` | 历史分析准确率回测验证 |
| 组合风险 | `/portfolio` | 投资组合 VaR、Sharpe、最大回撤分析 |
| 数据中心 | `/discover` | 技术指标、通知渠道、系统监控入口 |
| 系统设置 | `/settings` | API 密钥、模型参数、数据源配置管理 |

### 技术特点

- **前端**：React 19 + TypeScript + Vite + CSS Variables 双主题
- **后端**：FastAPI + SSE 实时推送 + 异步任务队列
- **API 代理**：Vite Dev Proxy，零 CORS 配置
- **响应式设计**：适配桌面与移动端
...
python main.py --webui-only  # 仅 Web 界面
...
## 📬 联系与合作
- GitHub Issues：[提交 Issue](https://github.com/sjkncs/daily_stock_analysis/issues)

## ⭐ Star History
**如果觉得有用，请给个 ⭐ Star 支持一下！**

<a href="https://star-history.com/#sjkncs/daily_stock_analysis&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=sjkncs/daily_stock_analysis&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=sjkncs/daily_stock_analysis&type=Date" />
   <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=sjkncs/daily_stock_analysis&type=Date" />
 </picture>
</a>
