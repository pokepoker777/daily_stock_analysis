...
class TradingCostConfig:
    ...
    Default values reflect typical retail costs:
    - Commission: 0.025% per side (negotiated rate, min 5 CNY)
    - Stamp tax: 0.05% sell-side only (reduced from 0.1% since Aug 2023)
    - Slippage: 0.05% per side (market impact estimate)
    """
    commission_rate: float = 0.00025   # 0.025% per side
    stamp_tax_rate: float = 0.0005     # 0.05% sell-side only
    slippage_rate: float = 0.0005      # 0.05% per side (market impact)

    @property
    def round_trip_cost_pct(self) -> float:
        """Total round-trip cost as a percentage."""
        buy_cost = self.commission_rate + self.slippage_rate
        sell_cost = self.commission_rate + self.stamp_tax_rate + self.slippage_rate
        return (buy_cost + sell_cost) * 100  # as %

    def apply_buy_cost(self, price: float) -> float:
        """Return effective entry price after buy-side costs (higher)."""
        return price * (1 + self.commission_rate + self.slippage_rate)

    def apply_sell_cost(self, price: float) -> float:
        """Return effective exit price after sell-side costs (lower)."""
        return price * (1 - self.commission_rate - self.stamp_tax_rate - self.slippage_rate)
...
class EvaluationConfig:
    ...
    trading_costs: TradingCostConfig = TradingCostConfig()
...
class BacktestEngine:
    ...
    def evaluate_single(
        ...
        *,
        ...
        simulated_return_pct: Optional[float]
        simulated_return_net_pct: Optional[float] = None
        round_trip_cost_pct: Optional[float] = None
        if position != "long":
            simulated_return_pct = 0.0
            simulated_return_net_pct = 0.0
            ...
            # Net return (after slippage + commission + stamp tax)
            costs = config.trading_costs
            effective_buy = costs.apply_buy_cost(start_price)
            effective_sell = costs.apply_sell_cost(simulated_exit_price)
            simulated_return_net_pct = (effective_sell - effective_buy) / effective_buy * 100
        ...
        return {
            ...
            "simulated_return_net_pct": simulated_return_net_pct,
