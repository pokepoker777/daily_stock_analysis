...
class StockAnalysisPipeline:
    ...
    def fetch_and_save_stock_data(
        ...
        try:
            ...
            try:
                ...
                validation = validate_dataframe(df, stock_code=code, source=source_name)
                if validation.has_critical:
                    logger.warning(f"[{code}] ⚠️ Data quality CRITICAL: {validation.critical_summary}")
                elif validation.warning_count > 0:
                    logger.info(f"[{code}] Data quality: {validation.warning_count} warnings")
                else:
                    logger.debug(f"[{code}] Data quality: OK")
            except Exception as ve:
                logger.debug(f"[{code}] Data validation skipped: {ve}")
    ...
    def analyze_stock(self, code: str, report_type: ReportType) -> Optional[AnalysisResult]:
        ...
        from src.log_context import correlation_ctx
        with correlation_ctx(stock_code=code) as cid:
            logger.info(f"[{code}] 开始分析 (cid={cid})")
            return self._run_analysis_inner(code, report_type, _analysis_start)

    def _run_analysis_inner(self, code: str, report_type: ReportType, _analysis_start: float) -> Optional[AnalysisResult]:
        """Inner analysis logic wrapped in correlation context."""
        try:
            ...
            # Step 3.5: Advanced technical indicators (Bollinger/ATR/OBV/KDJ/CCI)
            advanced_indicator_result = None
            try:
                from src.indicators import analyze_advanced_indicators
                adv_context = self.db.get_analysis_context(code)
                if adv_context and 'raw_data' in adv_context:
                    raw_data_adv = adv_context['raw_data']
                    if isinstance(raw_data_adv, list) and len(raw_data_adv) > 0:
                        import pandas as pd
                        df_adv = pd.DataFrame(raw_data_adv)
                        price = getattr(realtime_quote, 'price', None) if realtime_quote else None
                        advanced_indicator_result = analyze_advanced_indicators(df_adv, current_price=price)
                        logger.info(f"[{code}] Advanced indicators: score={advanced_indicator_result.advanced_score}, "
                                    f"volatility={advanced_indicator_result.volatility_level}, "
                                    f"kdj={advanced_indicator_result.kdj_signal}")
            except Exception as e:
                logger.warning(f"[{code}] Advanced indicator analysis failed: {e}")
            ...
            # Step 6: 增强上下文数据（添加实时行情、筹码、趋势分析结果、高级指标、股票名称）
            enhanced_context = self._enhance_context(
                ...
                trend_result,
                stock_name,  # 传入股票名称
                advanced_indicator_result=advanced_indicator_result,
            )
            ...
            try:
                ...
                elapsed = time.time() - _analysis_start
                status = "success" if result and result.success else "failed"
                metrics.inc("analysis_total", labels={"code": code, "status": status})
                metrics.observe("analysis_duration_seconds", elapsed, labels={"code": code})
            except Exception:
                pass
            ...
            try:
                from src.metrics import metrics
                elapsed = time.time() - _analysis_start
                metrics.inc("analysis_total", labels={"code": code, "status": "error"})
                metrics.observe("analysis_duration_seconds", elapsed, labels={"code": code})
            except Exception:
                pass
            return None
    ...
    def _enhance_context(
        ...
        将实时行情、筹码分布、趋势分析结果、高级指标、股票名称添加到上下文中
        ...
        Args:
            ...
            trend_result: 趋势分析结果
            stock_name: 股票名称
            advanced_indicator_result: AdvancedIndicatorResult (Bollinger/ATR/OBV/KDJ/CCI)
        ...
        # 添加高级技术指标 (Bollinger/ATR/OBV/KDJ/CCI/Williams%R)
        if advanced_indicator_result is not None:
            enhanced['advanced_indicators'] = advanced_indicator_result.to_dict()
            # Also store prompt-ready text for direct injection into AI prompt
            enhanced['advanced_indicators_text'] = advanced_indicator_result.to_prompt_text()
