...
5. **风险优先级**：舆情中的风险点要醒目标出"""
    ...
    def _call_api_with_retry(self, prompt: str, generation_config: dict) -> str:
        ...
        if self._use_openai:
            ...
            get_limiter("openai", max_tokens=20, refill_period=60.0).acquire()
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Throttle Gemini calls via token-bucket rate limiter
                from src.rate_limiter import get_limiter
                get_limiter("gemini", max_tokens=15, refill_period=60.0).acquire()
...
"""
        ...
        if 'advanced_indicators_text' in context:
            prompt += f"""
{context['advanced_indicators_text']}
"""
...
请输出完整的 JSON 格式决策仪表盘。"""
    ...
    def _parse_response(
        ...
        Uses Pydantic-validated parsing via src.schemas.ai_response for type safety.
        All fields receive sensible defaults if the AI output is partial or malformed.
        Falls back to keyword-based text extraction when JSON is entirely absent.
        """
        try:
            from src.schemas.ai_response import parse_ai_response

            # Step 1: Try Pydantic-validated JSON parsing
            validated = parse_ai_response(response_text)

            # Step 2: Override stock name if the original is a placeholder
            ai_stock_name = validated.stock_name
            if ai_stock_name and ai_stock_name != "未知":
                if name.startswith('股票') or name == code or 'Unknown' in name:
                    name = ai_stock_name

            # Step 3: Convert validated Pydantic model to AnalysisResult
            d = validated.to_legacy_dict()
            return AnalysisResult(
                code=code,
                name=name,
                ...
                trend_analysis=d["trend_analysis"],
                short_term_outlook=d["short_term_outlook"],
                medium_term_outlook=d["medium_term_outlook"],
                technical_analysis=d["technical_analysis"],
                ma_analysis=d["ma_analysis"],
                volume_analysis=d["volume_analysis"],
                pattern_analysis=d["pattern_analysis"],
                fundamental_analysis=d["fundamental_analysis"],
                sector_position=d["sector_position"],
                company_highlights=d["company_highlights"],
                news_summary=d["news_summary"],
                market_sentiment=d["market_sentiment"],
                hot_topics=d["hot_topics"],
                analysis_summary=d["analysis_summary"] or "分析完成",
                key_points=d["key_points"],
                risk_warning=d["risk_warning"],
                buy_reason=d["buy_reason"],
                search_performed=d["search_performed"],
                data_sources=d["data_sources"] or "技术面数据",
                success=True,
            )

        except Exception as e:
            logger.warning(f"Pydantic parsing failed ({e}), falling back to legacy parser")

        # Legacy fallback: manual JSON extraction
        try:
