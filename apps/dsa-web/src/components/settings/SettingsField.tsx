  ...
  return (
    <div className={`settings-field ${hasError ? 'settings-field-error' : ''}`}>
      <div className="settings-field-header">
        <label className="settings-field-label" htmlFor={`setting-${item.key}`}>
          {title}
        </label>
        {schema?.isSensitive ? (
          <span className="settings-field-badge">敏感</span>
        ) : null}
      </div>

      {description ? (
        <p className="settings-field-desc" title={description}>
          {description}
        </p>
      ) : null}
      ...
      {issues.length ? (
        <div className="settings-field-issues">
          {issues.map((issue, index) => (
            <p
              key={`${issue.code}-${issue.key}-${index}`}
              className={`settings-field-issue ${issue.severity}`}
            >
              {issue.message}
            </p>
          ))}
